...
export function VoiceCallsSection({ showHeading = true }: VoiceCallsSectionProps) {
  ...
  const handleNoiseGateThresholdChange: ChangeEventHandler<HTMLInputElement> = (evt) => {
    ...
    setCallMicNoiseGateThresholdDb(Math.max(-100, Math.min(0, threshold)));
  };
  const gateBarRef = useRef<HTMLDivElement | null>(null);
  const thresholdToPercent = (thresholdDb: number): number =>
    Math.max(0, Math.min(100, ((thresholdDb + 100) / 100) * 100));

  const setThresholdFromClientX = (clientX: number) => {
    const bar = gateBarRef.current;
    if (!bar) return;
    const rect = bar.getBoundingClientRect();
    const ratio = (clientX - rect.left) / rect.width;
    const clampedRatio = Math.max(0, Math.min(1, ratio));
    const threshold = Math.round(clampedRatio * 100 - 100);
    setCallMicNoiseGateThresholdDb(threshold);
  };

  const handleGateBarMouseDown: MouseEventHandler<HTMLDivElement> = (evt) => {
    evt.preventDefault();
    setThresholdFromClientX(evt.clientX);

    const onMove = (moveEvt: globalThis.MouseEvent) => {
      setThresholdFromClientX(moveEvt.clientX);
    };
    const onUp = () => {
  ...
  return (
    <Box direction="Column" gap="100">
      ...
      <Text size="L400">Noise Gate</Text>
      <SequenceCard className={SequenceCardStyle} variant="SurfaceVariant" direction="Column">
        <SettingTile
          title="Mic Noise Gate (Experimental)"
          description="Cuts out quiet background sounds and only lets your voice through when it's loud enough."
      ...
      <SequenceCard className={SequenceCardStyle} variant="SurfaceVariant" direction="Column">
        <SettingTile
          ...
          description="Drag the marker on the bar or type a number. Move it right to require a louder voice to be heard."
          after={
            <Box direction="Column" gap="100" alignItems="End">
              ...
              <Box
                ref={gateBarRef}
                onMouseDown={handleGateBarMouseDown}
                style={{
                  width: toRem(220),
                  position: 'relative',
                  height: toRem(12),
              ...
              >
                ...
                <div
                  style={{
                    ...
                    left: `calc(${thresholdToPercent(callMicNoiseGateThresholdDb)}% - ${toRem(2)})`,
                    background: 'rgba(15, 15, 15, 0.95)',
                    boxShadow: '0 0 0 1px rgba(255,255,255,0.65), 0 1px 6px rgba(0,0,0,0.35)',
                  }}
                />
              </Box>
              ...
              {micPreviewError !== '' && (
                <Text size="T200" style={{ maxWidth: toRem(260) }} priority="300">
                  Couldn't start mic preview: {micPreviewError}
                </Text>
              )}
            </Box>
          }
